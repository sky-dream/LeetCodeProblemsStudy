...
# https://leetcode-cn.com/problems/guess-the-word/solution/san-bu-xuan-ze-cai-na-ge-dan-ci-shang-jie-zui-xiao/
'''
朴素想法：
维护一个可行的单词列表，每次从可行单词中猜一个，通过距离排除部分可行单词，缩小可行单词列表。
但是这样可能会使得猜测次数比较多，所以需要有个保底的优化策略：
1. 对每个可行单词word，计算其他可行单词与其距离（0-6），保存在数组dis中。
2. 如果选择word进行猜测，猜测后剩余可行单词数量就是与word特定距离(猜测返回值)的可行单词数量
dis[master.guess(word)],剩余单词的上限为max(dis)，这是最差情况。
