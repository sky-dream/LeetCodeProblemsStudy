    ...
    def __init__(self, matrix):
        ...
        for i in range(maxRow):
            for j in range(maxCol):
                self.sumPoint[i+1][j+1] = self.sumPoint[i+1][j] + self.sumPoint[i][j+1] + matrix[i][j] - self.sumPoint[i][j]
...
def main():
    ...
    for i in range(1,len(operation)):
        ...
        cmd = 'obj.'+operation[i]+"(%r,%r,%r,%r)" % (indexGroup[i][0],indexGroup[i][1],indexGroup[i][2],indexGroup[i][3])
        result = eval(cmd)
        print("result: ",result)
