...
class Codec:
    ...
    def buildTree(self, s, start, end, start2end) -> Node:
        if s == '':
            return None
        # if the sub str s[start:end+1] is not start with "[", means it is a leaf node with no children.
        if s[start] != '[':
        ...
        while i <= end:
            if s[i].isnumeric():
                ...
                while s[j].isnumeric():
                    j += 1
                # the first one is the current root, the others are the children leaf nodes
                nodes.append(self.buildTree(s, i, j-1, start2end))
                i = j+1
            elif s[i] == '[':
                # add the root of the sub tree
